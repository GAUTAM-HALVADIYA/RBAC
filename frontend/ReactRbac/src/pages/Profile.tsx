...
export default function Profile() {
  const { profile, loading, error: fetchError, fetchProfile, handleUpdateProfile, handleUploadAvatar, handleDeleteAvatar } = useProfile();
      ...
      <div className="card glass-panel border-0 shadow-sm" style={{ maxWidth: '600px' }}>
        <div className="card-body p-4">
          ...
          <div className="d-flex align-items-center gap-4 mb-4">
            ...
            <div>
              <h2 className="fw-bold mb-1" style={{ color: 'var(--text-main)' }}>{profile?.user?.name}</h2>
          ...
          {isEditing ? (
            <form onSubmit={onUpdateSubmit}>
              ...
              <div className="d-flex gap-2">
                ...
                <button type="button" className="btn btn-light px-4" onClick={() => setIsEditing(false)}>Cancel</button>
              </div>
            </form>
          ) : (
            <div>
              ...
              <div className="mb-3">
                ...
                <p className="mb-0">{profile?.address || <span className="text-muted fst-italic">No address added</span>}</p>
              </div>
              <div className="mb-4">
                <p className="text-muted small fw-medium mb-1">Date of Birth</p>
                <p className="mb-0">
                  {profile?.dob ? new Date(profile.dob).toLocaleDateString() : <span className="text-muted fst-italic">Not specified</span>}
                </p>
              </div>
              <button className="btn btn-outline-primary px-4" onClick={() => setIsEditing(true)}>Edit Profile</button>
            </div>
          )}
        </div>
      </div>
