...
export default function Roles() {
  const { roles, loading, error, meta, page, setPage, fetchRoles, handleCreateRole, handleUpdateRole, handleDeleteRole } = useRoles();
        ...
        <div className="card-body p-0">
          <div className="table-responsive">
            <table className="table table-hover table-custom mb-0">
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan={2} className="text-center py-4"><div className="spinner-border text-primary" role="status"></div></td></tr>
                ) : roles.length === 0 ? (
                  <tr><td colSpan={2} className="text-center text-muted py-4">No roles available</td></tr>
                ) : (
                  roles.map((role) => (
                    <tr key={role._id}>
                      ...
                      <td className="align-middle text-end px-4">
                        <button className="btn btn-sm btn-light text-primary me-2 shadow-sm" onClick={() => handleEditClick(role)}>
                          <i className="bi bi-pencil-square"></i> Edit
                        </button>
                        <button className="btn btn-sm btn-light text-danger shadow-sm" onClick={() => onDeleteClick(role._id)}>
            ...
            <div className="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
              ...
              <form onSubmit={onSubmit}>
                <div className="modal-body px-4">
                  ...
                  <div className="mb-4">
                    <label className="form-label text-muted small fw-medium text-uppercase tracking-wide">Role Name</label>
                    <input 
                      type="text" 
                ...
                <div className="modal-footer bg-light bg-opacity-50 border-top-0 pt-3 pb-4 px-4">
                  ...
                  <button type="submit" className="btn btn-primary px-4 py-2 fw-medium shadow-sm">{isEditing ? 'Save Changes' : 'Create Role'}</button>
                </div>
              </form>
            </div>
          </div>
